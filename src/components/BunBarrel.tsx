...
export const BunBarrel:React.FC<{setCards:(tasks:TaskType[])=>void}>=({
  ...
  return (
    ...
    flex justify-center items-center rounded border text-3xl hover:bg-red-400 cursor-pointer ${
      active
      ? "border-red-800 bg-red-800 text-white"
      : "border-red-500/20 text-xl bg-[#000000ad] text-white"
    }
    `}>
      {active ? <Trash className='w-8 h-8'/> : <Trash className="animate-bounce w-8 h-8" />}
      <h2 className='text-sm'>Corbeille</h2>
